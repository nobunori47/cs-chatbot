...
> **🔮 将来構想** — FAQ テーブルを別途管理して関連情報を動的に注入する構成にすると、知識のメンテナンスが格段に楽になる。これは今後の改善点として取り組む予定だ。
...
**原因その 2**: それでも解決しなかった。`@supabase/postgrest-js` のバージョンアップに伴う**仕様変更**が原因で、`apikey` ヘッダーを `global.headers` に明示的に渡す必要があった。
...
**教訓**: 「動いていたコードが突然壊れる」はライブラリの仕様変更のサイン。まず **Changelog と `package.json` のバージョン**を確認する習慣が大切だ。
...
Claude は会話の文脈を「messages 配列」で受け取る。配列の最後が `role: "user"` である必要があるが、今回のフローでは「ユーザーメッセージを INSERT してから会話履歴を SELECT する」順序なので、自然に満たされる。

なお `role` は `"user"` と `"assistant"` の 2 値しか存在しないため、DB に `sender_type: 'operator'` のメッセージがある場合は除外して渡す必要がある（現在は未実装・今後対応予定）。
...
今回の案件を通して、Next.js・Supabase・Claude API を組み合わせたフルスタック開発を経験し、Realtime・RLS・AI 応答・管理画面まで含めた実践的な CS チャットボットを完成させることができました。特に「動けば終わりではなく、安全に運用できる設計まで考えること」の重要性を実感できたことが、一番の学びでした。
